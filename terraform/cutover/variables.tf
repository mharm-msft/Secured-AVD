# =============================================================================
# variables.tf (CUTOVER) — 34 canonical + 3 cutoverOnly parameters.
#
...
#
# CIDR defaults shifted by /16 to avoid colliding with the greenfield
# example when deployed side-by-side in the same subscription.
# =============================================================================
...
# ---- 9. Cutover-only ---------------------------------------------------------
# Resource IDs of the OLD AVD stack we are replacing. The cutover deployment
# does NOT mutate these resources — it deploys a parallel new stack and emits
# a drain runbook the operator runs at switchover time.
variable "existingHostPoolResourceId" {
  type        = string
  description = "Resource ID of the OLD AVD host pool. Format: /subscriptions/<subId>/resourceGroups/<rg>/providers/Microsoft.DesktopVirtualization/hostPools/<name>"
  validation {
    condition     = can(regex("^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft\\.DesktopVirtualization/hostPools/[^/]+$", var.existingHostPoolResourceId))
    error_message = "existingHostPoolResourceId must be a full ARM resource ID for a Microsoft.DesktopVirtualization/hostPools resource."
  }
}

variable "existingWorkspaceResourceId" {
  type        = string
  description = "Resource ID of the OLD AVD workspace. Format: /subscriptions/<subId>/resourceGroups/<rg>/providers/Microsoft.DesktopVirtualization/workspaces/<name>"
  validation {
    condition     = can(regex("^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft\\.DesktopVirtualization/workspaces/[^/]+$", var.existingWorkspaceResourceId))
    error_message = "existingWorkspaceResourceId must be a full ARM resource ID for a Microsoft.DesktopVirtualization/workspaces resource."
  }
}

variable "drainOldHostPool" {
  type        = bool
  description = "When true, the deployment outputs include a ready-to-run PowerShell snippet that drains user sessions from the OLD host pool. The snippet is NEVER auto-executed."
  default     = true
}
